...
```bash
docker run [...] -v <PATH>/config.json:/config.json ghcr.io/mfernandezfunes/watchtower
```
...
```bash
docker run [...] -v <PATH_TO_HOME_DIR>/.docker/config.json:/config.json ghcr.io/mfernandezfunes/watchtower
```
