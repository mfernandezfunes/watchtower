...
-   **Monitor only**: In this mode, watchtower checks for container updates, sends notifications and invokes the [pre-check/post-check hooks](https://mfernandezfunes.github.io/watchtower/lifecycle-hooks/) on the containers but does **not** perform the update.
...
If you wish to create a monitoring scope, you will need to [run multiple instances and set a scope for each of them](https://mfernandezfunes.github.io/watchtower/running-multiple-instances).
...
When the label is specified on a container, watchtower treats that container exactly as if [`WATCHTOWER_MONITOR_ONLY`](https://mfernandezfunes.github.io/watchtower/arguments/#without_updating_containers) was set, but the effect is limited to the individual container. 
